...
func main() {
	...
	for {
		...
		view.ShowMenuInConsole()

		selectedOption := view.ReadOptionFromConsole()

		if validation.IsAValidOption(selectedOption) {

			if !validation.IsExitSelected(selectedOption) {
...
func playGame(selectedOption int) {
	...
	if selectedOption == 3 {
		...
		fmt.Printf(c.FACTORIAL_CALCULATING, num)

		fmt.Printf("Result is: %d\n\n", games.FactorialOf(num))
