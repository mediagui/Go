...
func main() {
	...
	valorMultiplicador, valorCombo, valorListaPuntos := registraParametrosEntrada()

	RegistrarPuntos(func(p int) int {
		return p - 10
	}, valorListaPuntos...)

	funcionMultiplicador := NuevoMultiplicador(*valorMultiplicador)

	println("Doble puntos:", funcionMultiplicador(50))

	println("Combo de", *valorCombo, ":", SumaCombo(*valorCombo))
}
...
func registraParametrosEntrada() (*int, *int, []int) {
	puntos := flag.String("puntos", "0", "Números enteros separados por comas. Deben ir al final")
	mult := flag.Int("mult", 1, "Valor por el que multiplicar los puntos")
	combo := flag.Int("combo", 0, "Cantidad de golpes seguidos")

	flag.Parse()

	listaPuntos := convierteAEnteros(strings.Split(*puntos, ","))
	return mult, combo, listaPuntos
}

func convierteAEnteros(puntos []string) []int {
	var listaPuntos []int
	for _, arg := range puntos {
		if num, err := strconv.Atoi(strings.TrimSpace(arg)); err == nil {
			listaPuntos = append(listaPuntos, num)
		}
	}
	return listaPuntos
}
