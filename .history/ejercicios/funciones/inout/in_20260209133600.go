...
func PedirInfo(user *dto.UsuarioStruct) {
	...
	for _, campoAGuardar := range camposDisponibles {
		...
		log.Printf("Pidiendo campo %s.", campoAGuardar)
...
func guardaCampoEn(k string, v string, user *dto.UsuarioStruct) {
	...
	nombreMetodo, metodo := metodoAInvocar(k, userValue)

	// Guardamos el valor en el dto
	guardaElValorEnDto(metodo, v, nombreMetodo)
...
func metodoAInvocar(k string, userValue reflect.Value) (string, reflect.Value) {
	...
	log.Println("Método a invocar: [%s].", methodName)
