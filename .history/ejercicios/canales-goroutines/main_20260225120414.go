...
func main() {
	...
	numeroGoruintas := runtime.NumCPU() * 10000
	...
	var totalSuma *int
	totalSuma = calculaSuma(numeroGoruintas, resultados, &totalSuma)

	fmt.Printf("\nSuma total de los %d resultados: %d\n", numeroGoruintas, *totalSuma)
}

func calculaSuma(numeroGoruintas int, resultados chan int, totalSuma *int) int {
