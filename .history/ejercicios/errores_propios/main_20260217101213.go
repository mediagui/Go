...
func saveContactsToAgenda(contacts map[int]contactStruct, agenda *os.File) {
	...
	agenda, err := os.OpenFile(fmt.Sprintf("%s%sagenda.txt", path, string(os.PathSeparator)), os.O_APPEND|os.O_WRONLY, 0644)
...
func createAgenda() *os.File {
	...
	fichero, error := os.Create(fmt.Sprintf("%s%sagenda.txt", path, string(os.PathSeparator)))
...
func openAgenda() *os.File {
	...
	agenda, err := os.OpenFile(fmt.Sprintf("%s%sagenda.txt", path, string(os.PathSeparator)), os.O_RDONLY, 0644)
	checkError(err)
