...
func main() {
	...
	howManyDays := askIntInRange("¿Cuántos días quieres cambiar (0-7)? ", 0, 7)
...
func requestUserTheDayToChange() dayToChange {
	...
	idx := askIntInRange("Elije el día a cambiar en el mapa "+weekDaysDisplay+" (1-7): ", 1, 7)
...
func getWeekDaysDisplay() string {
	...
	for i := 1; i <= 7; i++ {
		...
		s += fmt.Sprintf("%d:%s", i, time.Weekday(i%7).String())
	}
	return s
}
...
func printMap(weekDaysMap map[int]string) {
	// Imprimir el mapa mostrando las claves como 1..7 para el usuario,
	// mientras que internamente las claves son 0..6.
	fmt.Print("Nuevo mapa: map[")
	for i := 0; i <= 6; i++ {
		if i > 0 {
			fmt.Print(" ")
		}
		fmt.Printf("%d:%s", i+1, weekDaysMap[i])
	}
	fmt.Println("].")
}
