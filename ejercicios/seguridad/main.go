...
func main() {
	...
	numHilosALanzar := 1000

	wg := sync.WaitGroup{}
	wg.Add(numHilosALanzar)
	...
	lanzaEscritura(numHilosALanzar, &wg, &mutex)
	wg.Wait() //Esperamos a que las goroutines terminen en vez de usar Time.sleep

	log.Println("Fin")
}

func lanzaEscritura(numHilosALanzar int, wg *sync.WaitGroup, mutex *sync.Mutex) {
	for i := range numHilosALanzar {
		go func(i int) {
			log.Println("Ejecutando hilo[", i, "]")
			defer wg.Done()
			guardaEnArchivo(eventos, mutex)
		}(i)
	}
}
...
func guardaEnArchivo(ev [5]Acceso, m *sync.Mutex) {
	...
	for i := range 5 {
		...
		//El formato de la linea escrita debe ser: [ID: 001] - Usuario: Sergio - RIESGO: ATAQUE ATAQUE_CRITICO
		registro := fmt.Sprintf(
			"[%03d] - Usuario: %s - RIESGO: %s - %v\n",
			ev[i].Id,
			ev[i].Usuario,
			ev[i].Categoria,
			ev[i].Riesgo,
